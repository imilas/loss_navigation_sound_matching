...
\begin{equation}
    ...
    = \left\| S(x_{\theta}) - S(x^{t}) \right\|_{1}.
\end{equation}

\medskip
\noindent\textbf{\SIMSESpec:}
The scale-invariant mean-squared error (SIMSE)


\medskip
\noindent\textbf{\JTFS:}
The joint time-frequency scattering (JTFS) transform provides a multiresolution representation of temporal and spectral structure~\cite{vahidi2023mesostructures,anden2015joint}. The loss is defined as:
\begin{equation}
...
This loss computes a dynamic time-warping (DTW) distance between amplitude envelopes:
\begin{equation}
...
Full implementation details and parameter settings follow the configurations introduced in~\cite{salimi2025evaluating}.
