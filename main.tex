...
\end{equation}
where $\theta^{(\mathrm{crit})}$ denotes the target’s critical parameters (e.g., filter cutoffs, modulation rates) and $\hat{\theta}^{(\mathrm{crit})}$ the corresponding recovered parameters of the imitator.  In OOD scenarios, some level of subjectivity is unavoidable in selecting such critical parameters. However, the scenarios here's have been designed with a single, targeted focus in order to make the PPL definition fairly straightforward.

Parameters with no perceptual or structural correspondence are ignored. 
If done correctly, this can enable a reasonable interpretable comparison across mismatched synthesizers.  The exact parameters used for each scenario will be discussed in their corresponding subsection later on in Section~\ref{sec:scenarios}.
...
\todo{maybe this should be in results}Moderate to strong correlation was observed between the listeners' likert scores, using speraman's rank correlation test~\cite{spearman1987proof}. This strengthens our confidence in using human assigned scores as the ground truth. Figure~\ref{tab:listener_correlation} shows the individual and overall $\rho$ and p-values.
