...
The methodology evaluates loss functions across multiple out-of-domain (OOD) sound-matching scenarios. Each experiment follows the iterative optimization loop as shown in Figure~\ref{fig:sound_design_loop_iterative}, where a synthesizer $g$ is optimized to imitate a target sound generated by a different synthesizer $g^{t}$. In other words, an experiment is a search for parameters that minimize the difference between the output of a synthesizer to a sound that is the output of another synthesizer. Each experiment consists of random parameter initialization, 200 steps of gradient-based optimization, and final parameter evaluation (also known as the performance score).

A \textit{scenario} consists of four experiment results---one for each of the four loss functions---performed with the same pair of synthesizers $(g, g^{t})$. Each scenario models a distinct sound-design task, such as matching filter cutoffs, modulation structure, or pitch trajectories, allowing us to evaluate how effective each loss function is for under specific forms of OOD mismatch.
...
To determine which loss function is the best performer for which scenario, the experiments must be evaluated and given a score. 

 We have two evaluation methods: the automatic partial P-Loss (PPL), which we introduce in this paper as a reasonable measure of performance in OOD settings, and similarity ratings (or likert scores) assigned by two of the authors.
...
\label{sec:scenarios}
\todo{mention sampling rate is set to 48000}
