...
\node (L) [above of=start, right of=start, text centered] 
    {\( L(\theta, x^t) \propto \phi(x_{\hat{\theta}}) - \phi(x^t) \)};
...
\section{methodology}
The foundation of our experiments is iterative loop shown in Figure~\ref{fig:sound_design_loop_iterative}. This iterative approach is the OOD adaption of previous work by Salimi \textit{et al.}~\cite{salimi2025evaluating}. The goal of the experiments here is to measure the performance of loss functions in different 

Each experiment having 3 phases:
\begin{enumerate}
    \item Set a
\end{enumerate}
