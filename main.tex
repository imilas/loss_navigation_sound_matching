 ...
 \begin{figure*}[tbp]
  ...
  % 1st row
    \begin{minipage}{\textwidth}
      ...
      \end{minipage}%
      \begin{minipage}{0.96\textwidth}\centering
        \includegraphics[width=0.45\textwidth]{images/npsk_ood_P_Loss_4.png}%
        \hspace{0.03\textwidth}%
      \end{minipage}
    \end{minipage}
     \caption{Bootstrapped distributions and ranks for delayed pitch-bending}
  \label{fig:npsk_pitch_bend_up_delayed}
...
\begin{figure*}[t]
  ...
  \begin{minipage}[t]{\textwidth}
    ...
    \begin{minipage}[t]{0.045\textwidth}
      \footnotesize\raggedleft
      \vspace{-4.5cm} % align with top of images
      SIMSE\\[0.4cm]
      L1\\[0.385cm]
      JTFS\\[0.365cm]
      DTW
    ...
    \begin{minipage}[t]{0.91\textwidth}
      ...
      \begin{minipage}[t]{0.45\textwidth}
        ...
        \vspace{0.3em}
        \footnotesize Non-delayed Pitch-bend
      \end{minipage}
...
Here, we can define success with P-Loss, that is, how well the \texttt{increase\_speed} and \texttt{starting\_pitch} parameters are approximated despite the delay in the pitch-bending onset. Figure~\ref{fig:npsk_pitch_bend_up_delayed} shows that \JTFS is the best performing loss function, which is consistent with the previous findings by Vahidi \textit{et al.}~\cite{vahidi2023mesostructures}. 

An interesting follow-up experiment conducted was the \textit{in-domain} comparison of pitch-bending synthesizers \textit{without} delay. This is identical to the delayed pitch-bending experiment but setting the delay variable $\delta=0$. 
