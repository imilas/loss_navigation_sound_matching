...
Manual sound-design is a feedback loop built on the designer hearing an output and measuring the similarity of what is currently generated versus what they would like to hear. This subjective, internal measurement is impossible to recreate between people with similar tastes, let alone digitally. While impractical in large scales, manual listening tests by one or more people can be used to measure the success of sound-matching experiments. Over the years, many automatic alternatives have been defined in previous sound-matching experiments, but results have generally viewed with a large degree of skepticism~\todo{cite}.


Any quantification of similarity between two sounds can be used as a performance measure, that is, any function that can be used as a loss function can also be used as a performance measure. Although we note that the use of the same function measure loss and performance is not a good practice. Minimizng a loss function and then measuring success of the experiment on how a loss function does not 

\section{methodology}
The foundation of our experiments is iterative loop shown in Figure~\ref{fig:sound_design_loop_iterative}. This iterative approach is the OOD adaption of previous work by Salimi \textit{et al.}~\cite{salimi2025evaluating}. The goal of the experiments here is to measure the performance of loss functions in different OOD scenarios. In each scenario, a large number of experiments with different loss functions, but the synthesizers are unchanged. 
...
\begin{enumerate}
    \item $L$ and the scenario's synthesizers $g$ and $g^t$, create a target sound $x^t$ by with random parameters $\theta^t$ given to $g^t$. $g$ is also randomly initiated with $\theta_n$, where $n$ indicates the number of iterations, beginning at $n=0$
