...
\section{Background}
\subsection{Defining Sound-Matching}
 Formal definitions of sound-matching given in previous work require some modifications in order to be useful in OOD scenarios~\cite{vahidi2023mesostructures,han2023perceptual,salimi2025evaluating}. Adapting the definitions from Salimi \textit{et al.} gives us the following key components:
\begin{itemize}
    \item $g(\theta)$: A parametric audio synthesizer $g$ with parameters $\theta$.
    \item $g^{t}(\theta^t)$: The synthesizer which outputs the target sound $x^t$. This synthesizer is either different from $g$, or it has non-overlapping parameter ranges.
    \item $x_\theta$: The synthesizer output, $x_\theta = g(\theta)$”.
    \item $x_0$: The target sound to be replicated or imitated. 
    \item $\phi(\cdot)$: A representation (feature extraction) function mapping signals into a comparison space.
    \item $L$: A loss function that measures distance between $x_\theta$ and $x_0$, typically via $L(\theta,x_0) = d(\phi(x_\theta), \phi(x_0))$ for some metric $d$.
\end{itemize}
...
% In-domain experiments have shown that in a differentiable, iterative sound-matching setting, different loss functions appear to be appropriate for different methods of synthesis~\cite{salimi2025evaluating}. This work is the further analysis of the interplay between the same loss functions and synthesizers in out-of-domain settings. 
...
The iterative loop shown in Figure~\ref{fig:sound_design_loop_iterative} is the building block of our analysis. In each trial, we select a target synthesizer $g_t$, and generate the target sound $x$ with randomly set parameters $\theta_t$. We then select a synthesizer $g$--often not equal to $g_t$ since these are out-of-domain experiments--and loss function $L$, and the iterative loop is applied for 200 iterations, after which the the partial P-Loss for $\theta_199$ is recorded as the measure of the trial's success. What we call an experiment is 300 trials with the same $L$, $g$, and $g_t$, giving us 300 measures of success. We design sound-matching scenarios by keeping $g$ and $g_t$ the same, and conducting experiments with each $L$. This gives four distributions of partial P-Loss values for each loss function. Comparing the four distributions highlights the most appropriate loss function for a specific scenario. 

Like previous work~\cite{salimi2025evaluating}, the distributions are upsampled to 1000 values using bootstrapping (1000 samples of the mean using 100\% of the values with replacement)~\cite{tibshirani1993introduction,chernick2011bootstrap}. For each scenario, the loss functions are ranked using the non-parametric Scott-Knott (\gls{NPSK}) test~\cite{tantithamthavorn2017mvt,tantithamthavorn2018optimization}, which ranks the loss functions from rank 1 (best, or highest distribution values) to maximum rank of 4, with the possibility of ties between distributions. 

To summarize, a trial is 200 iterations of a loop with the final partial P-Loss used as the performance measure, an experiment is 300 trials, a scenario is 4 experiments with the same synthesizers and different loss functions. 

\subsection{Partial P-Loss}
