% this paper verifies the utility of loss functions previously not tested in OOD settings. We define sound-matching tasks with targets that cannot be matched by the synthesizer. This target sound is from a target synthesizer which has at least 1 parameter in common with the output synthesizer. For each sound-matching task, we use a bespoke partial parameter loss (PPL) that measures the difference between the synthesizer parameters of interest. 
